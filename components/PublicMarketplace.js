...
import {
  ...
  Award, Sparkles, Camera, Phone, Mail, Check, Zap, Car
} from 'lucide-react'
...
export default function PublicMarketplace({ session, profile, onOpenAuth, onOpenPortal, onLogout }) {
  ...
  const doSearch = useCallback(async (pg, overrideTab) => {
  ...
  /* ═══ DETAIL VIEW ═══ */
  if (sel) {
    return <DetailView v={sel} session={session} profile={profile} rate={rate}
  ...
  return (
    <div className="paper-grain" style={{ minHeight: '100vh', position: 'relative' }}>
      ...
      {/* Compare tray */}
      {compare.length > 0 && (
        <div style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', background: 'var(--ink)', color: 'var(--paper)', borderRadius: 14, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: 'var(--shadow-xl)', zIndex: 80 }}>
          <GitCompare size={16} />
          ...
          <button className="btn-crimson" style={{ height: 32, padding: '0 14px', fontSize: 12 }}>Compare →</button>
        </div>
      )}
    </div>
  )
...
/* ════════════════════════════════════════════════════════════
   HERO
   ════════════════════════════════════════════════════════════ */
function Hero({ f, up, setF, modelOpts, showFilters, setShowFilters, doSearch, session, onOpenAuth }) {
  return (
    <section style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', maxWidth: 1240, margin: '0 auto', padding: '90px 24px 140px', zIndex: 2 }}>
        ...
        {/* Search card */}
        <div className="fade-up" style={{ animationDelay: '180ms', background: 'rgba(250,248,244,.98)', backdropFilter: 'blur(20px)', borderRadius: 20, padding: 18, border: '1px solid rgba(255,255,255,.3)', boxShadow: 'var(--shadow-xl)', maxWidth: 980 }}>
